...
def doc_to_text(doc: dict, model_specific_kwargs: dict) -> str:
    """Process Food101 document to extract question text.
...
def doc_to_text_multi_round(
    ...
    """Process Food101 document to extract information for multi-round dialogue.
...
def doc_to_visual(doc: dict) -> list:
    """Convert Food101 document to visual format.
...
def doc_to_target(doc: dict) -> str:
    """Convert Food101 document to target format.
...
def download(output_dir: str = "data", cache_dir: str = ".cache") -> datasets.DatasetDict:
    """Download Food101 dataset from the web and convert it to the HuggingFace format.
    ...
    dataset_path = Path(cache_dir, "data", "Food101")
    ...
    if not dataset_path.exists():
        ...
        images_path = Path(cache_dir, "data", "food-101")
        images_path.rename(dataset_path)

    # Read the dataset
    dataset_path = Path(cache_dir, "data", "Food101", "images")
