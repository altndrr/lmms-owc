...
def doc_to_text(doc: dict, model_specific_kwargs: dict) -> str:
    """Process OxfordPets document to extract question text.
...
def doc_to_text_multi_round(
    ...
    """Process OxfordPets document to extract information for multi-round dialogue.
...
def doc_to_visual(doc: dict) -> list:
    """Convert OxfordPets document to visual format.
...
def doc_to_target(doc: dict) -> str:
    """Convert OxfordPets document to target format.
...
def download(output_dir: str = "data", cache_dir: str = ".cache") -> datasets.DatasetDict:
    """Download OxfordPets dataset from the web and convert it to the HuggingFace format.
    ...
    dataset_path = Path(cache_dir, "data", "OxfordPets")
    output_path = Path(output_dir) / "oxford_pets"
    ...
    if not dataset_path.exists():
        ...
        images_path = Path(cache_dir, "data", "images")
