...
def doc_to_text(doc: dict, model_specific_kwargs: dict) -> str:
    """Process DTD document to extract question text.
...
def doc_to_text_multi_round(
    ...
    """Process DTD document to extract information for multi-round dialogue.
...
def doc_to_visual(doc: dict) -> list:
    """Convert DTD document to visual format.
...
def doc_to_target(doc: dict) -> str:
    """Convert DTD document to target format.
...
def download(output_dir: str = "data", cache_dir: str = ".cache") -> datasets.DatasetDict:
    """Download DTD dataset from the web and convert it to the HuggingFace format.
    ...
    """
    data_url = "https://www.robots.ox.ac.uk/~vgg/data/dtd/download/dtd-r1.0.1.tar.gz"
    dataset_path = Path(cache_dir, "data", "DTD")
    output_path = Path(output_dir) / "dtd"
    ...
    if not dataset_path.exists():
        ...
        images_path = Path(cache_dir, "data", "dtd")
        images_path.rename(dataset_path)

    # Read the dataset
    dataset_path = Path(cache_dir, "data", "DTD", "images")
